...
This guide explains how to use Global Tokens for authentication with Glean's API. Unlike User Tokens, Global Tokens can authenticate requests on behalf of any user in your organization. The guide provides step-by-step instructions for configuring proper token permissions and making authenticated API requests with the required X-Scio-Actas header.
